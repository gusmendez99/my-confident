$(document).ready(function () {
	...
	$("#create_account").submit(function (e) {
		...
		if (username.length === 0 || username.length > 32) {
			error =
				"Invalid username. Must be nonempty and contain at most 32 characters.";
		}
		if (
			username.match(/^[a-zA-Z0-9_-]+$/) == null ||
			username.match(/^[a-zA-Z0-9_-]+$/)[0] !== username
		) {
			error =
				"Invalid username. Must contain only letters (a-z), numbers (0-9), dashes (-), underscores (_).";
		}
		if (!password.match(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}$/)) {
			error =
				"Invalid password. Must be at least 8 characters and contain a lowercase letter, an uppercase letter, and a digit.";
		}
		if (error) {
		...
		var user_data = {
			username: username,
			public_key: pub_serialized,
			secret_key: sec_serialized,
		};
		var encrypted_user_data = sjcl.encrypt(
			password,
		...
		$.post($SCRIPT_ROOT + "/sign-up", req_data, function (data) {
			if (data.error) {
				$("#create_account")[0].reset();
				alert(data.error);
				return;
